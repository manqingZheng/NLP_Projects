...
def learn_trigram(data, backoff, delta, smoothing, para2,verbose=True):
    from lm import Trigram
    trigram = Trigram(backoff, delta, smoothing, para2)
    ...
    if verbose:
        ...
        # print('bbb')
        # print("sample 1: ", " ".join(str(x) for x in sampler.sample_sentence(['The','University'])))
        # print('ccc')
        # print("sample 2: ", " ".join(str(x) for x in sampler.sample_sentence(['Tomorrow','will','be'])))
        return trigram
...
if __name__ == "__main__":
    ...
    x_axis = [0.1, 0.3, 0.5, 0.7, 0.9]
    for i in x_axis:
        # model_new = models[2]
        # partial_data = datas[test_index].train[:,int(i*l)]
        data_test = datas[1]
        partial_data = data_test.train[:int(i*len(data_test.train))]
        print(type(partial_data))
        # print(datas[2].train)
        l = datas[2].train
        l.extend(partial_data)
        print(type(l), 'l')
        print(type(datas[1].train))
        model_new= learn_trigram(l,0.000001,0.01,'True',4)
        # model_new.fit_corpus(partial_data)
        print(model_new.perplexity(data_test.test))