...
if __name__ == "__main__":
    ...
    for dname in dnames:
        print("-----------------------")
        print(dname)
        data1 = read_texts("data/corpora.tar.gz", dname)
        datas1.append(data1)
        model1 = learn_unigram(data1)
    ...
    for i in xrange(n):
        for j in xrange(n):
            perp_dev[i][j] = models1[i].perplexity(datas1[j].dev)
            perp_test[i][j] = models1[i].perplexity(datas1[j].test)
            perp_train[i][j] = models1[i].perplexity(datas1[j].train)
