...
def viterbi_algo(list_word, dict_n_gram, dict_word, dict_emission, lam):
    tag_set = ('O', 'I-GENE')
    bp_dict = defaultdict()
    pi_dict = {(1, '*', '*'): 1}
    list_word = ['*','*'] + list_word
    for k in range(2, len(list_word)):
        ...
        for u, v in itertools.product(U, V):
            ...
            e =  (float(dict_emission[(v, x)])+lam) / (float(dict_n_gram[(v,)]) + lam * 2)
            pi_list = [((pi_dict[k-1, w, u] * ((dict_n_gram[w,u,v]+lam)/ (float(dict_n_gram[w,u]) + lam * 2)) * e), w) for w in W]
...
def write_to_genefile(corpus_file, output_file, dict_n_gram, dict_word, dict_emission,lam):
    list_word = []
    for l in corpus_file:
        line = l.strip()
        if line:
            ...
            list_tag = viterbi_algo(list_word, dict_n_gram, dict_word, dict_emission,lam)
...
if __name__ == "__main__":
    ...
    input.close()
    lam = 0.5
    write_to_genefile(input2,sys.stdout, dict_n_gram, dict_word, dict_emission,lam)
    input2.close()
